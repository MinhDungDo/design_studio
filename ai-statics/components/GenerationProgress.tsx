...
const STEPS = [
  { key: "brain", label: "Strategy & copy", desc: "Claude (BRIEF.md)" },
  { key: "imageGen", label: "Rendering", desc: "Higgsfield gpt-image-2" },
] as const;

export default function GenerationProgress({ steps, title }: Props) {
  const statusOf = (key: string): "waiting" | "running" | "done" => {
    const found = steps.find((s) => "step" in s && s.step === key);
    return found && "status" in found ? found.status : "waiting";
  };

  return (
    <div>
      {title && (
        <h3 className="text-[11px] font-medium text-white/40 uppercase tracking-widest mb-2">{title}</h3>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {status === "running" && <p className="text-[10px] text-white/40 mt-0.5">{desc}</p>}
              </div>
            </div>
          );
        })}
